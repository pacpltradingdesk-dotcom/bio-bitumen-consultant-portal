...
# ── STEP 2: FEASIBILITY ─────────────────────────────────────────────
    ...
    if selected_type == "new_investor":
        ...
        st.page_link("pages/03_Location.py", label="📍 Location Feasibility (18 States Scored)", icon="📍")
        st.page_link("pages/04_Raw_Material.py", label="🌾 Raw Material Analysis", icon="🌾")
        st.markdown(f"""
        **For your {cfg['capacity_tpd']:.0f} TPD plant:**
        ...
        """)
        st.page_link("pages/04_Raw_Material.py", label="🌾 See Biomass Availability Data", icon="🌾")
    elif selected_type == "pyrolysis_owner":
        st.markdown("""
...
# ── STEP 3: TECHNICAL SOLUTION ───────────────────────────────────────
    ...
    if selected_type == "new_investor":
        ...
        st.page_link("pages/05_Plant_Design.py", label="⚙️ Full Plant Design (5-100 TPD)", icon="⚙️")
    ...
    st.page_link("pages/06_Drawings.py", label="📐 View All Engineering Drawings (117)", icon="📐")
...
# ── STEP 6: COMPLIANCE ───────────────────────────────────────────────
    ...
    st.page_link("pages/09_Compliance.py", label="📋 Full Compliance Tracker (25 Licenses)", icon="📋")
...
# ── STEP 8: FINAL OUTPUT / PROFIT ────────────────────────────────────
    ...
    **Timeline:** {'12-18 months' if selected_type in ('new_investor', 'biomass_company') else '30-90 days'}
    ...
    doc_cols[1].page_link("pages/07_Financial.py", label="💰 Financial Model", icon="💰")
    doc_cols[2].page_link("pages/06_Drawings.py", label="📐 All Drawings", icon="📐")
    doc_cols[3].page_link("pages/16_AI_Advisor.py", label="🤖 Ask AI", icon="🤖")
