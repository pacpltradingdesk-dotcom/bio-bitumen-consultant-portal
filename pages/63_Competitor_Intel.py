...
# ── AI Skill: Competitive Positioning ──────────────────────────────────────
st.markdown("---")
try:
    from engines.ai_engine import is_ai_available, ask_ai
    if is_ai_available():
        with st.expander("🤖 AI: Competitive Positioning"):
            if st.button("Generate", type="primary", key="ai_63Compe"):
                with st.spinner("AI working..."):
                    _p = f"As a senior bio-bitumen consultant, generate: Competitive Positioning. "
...
with _ex1:
    if st.button("Download Excel", type="primary", key="exp_xl_63Com"):
        try:
            import io
            ...
            st.download_button("Download", _buf.getvalue(), "export.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_xl_63Com")
        except Exception as _e:
            st.error(f"Export failed: {_e}")
with _ex2:
    if st.button("Print", key="exp_prt_63Com"):
